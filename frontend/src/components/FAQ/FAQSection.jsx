    ...
    return (
        <section id="faq-section" ref={ ref }>
            <div>
                ...
                <p style={{...startingStyle,...(inView? changedStyle : {} )}}>
                    Below we’ve provided answers to the most frequently asked questions. 
                    If you have any other questions, please get in touch.
                </p>
            </div>
            {
                faqs.filter((faq, id)=> id < 1).map((faq)=> {
                   return faq.faqs.map(({ question, answer }, id) => <FAQ  id={ id } inView={ inView } question={ question } answer={ answer } expandible={ "false" }/>)    
                })
            }
            <button style={{ 
                                padding: "10px 0px", 
                            ...
                            }}><Link to="/faq">More Questions</Link></button>
        </section>
    )
}